"""
Hard-split Bi-LSTM: TabPFN vs XGBoost resource predictions.
Uses cached TabPFN predictions from checkpoints/tabpfn_resource_cache.json.
Same 80/10/10 split, same Bi-LSTM architecture as xgboost_bilstm.py.
"""
...
DIM_LABEL_KEYS = {'mem': 'memory_bytes', 'disk': 'disk_bytes',
...
def make_tabpfn_predictor(cache_path):
    """Load cached TabPFN predictions."""
    with open(cache_path) as f:
        cache = json.load(f)
    print(f'  Loaded TabPFN cache: {len(cache)} queries')
    def predict(qid):
        return cache.get(qid)
    return predict


def make_xgboost_predictor(ndv_cache, seed=42):
    """Train XGBoost on all cgroup data (same as xgboost_bilstm.py)."""
    ...
    def predict(qid):
        ...
        feat_n = (feat - Xm) / Xs
        return {DIM_NAMES[i]: float(models[i].predict(feat_n.reshape(1, -1))[0])
                for i in range(len(DIM_NAMES))}
...
def main():
    ...
    print('=' * 60)
    print('TabPFN vs XGBoost → Bi-LSTM (Hard Split)')
    ...
    # 80/10/10 query-level hard split
    ...
    for model_name in ['TabPFN', 'XGBoost']:
        ...
        # Stage 1: Resource Predictor
        print(f'\n[Stage 1] {model_name} resource predictor...')
        if model_name == 'TabPFN':
            cache_path = os.path.join(ROOT, 'checkpoints/tabpfn_resource_cache.json')
            predictor = make_tabpfn_predictor(cache_path)
        else:
            predictor = make_xgboost_predictor(ndv_cache, seed=args.seed)
        ...
        all_results[model_name] = {
            ...
            'P10': qe[int(n*0.1)], 'P99': qe[int(n*0.99)]
        }
        seq_info[model_name] = {'dim': d_in, 'params': n_params, 'n_test': n}

    # ═══ Comparison ═══
    print('\n' + '=' * 60)
    print('FINAL COMPARISON: Stage 1 → Bi-LSTM Concurrent Prediction')
    print(f'80/10/10 hard split, seed={args.seed}, epochs={args.epochs}')
    print('=' * 60)
    ...
    print(f'  Seq dim: {seq_info["TabPFN"]["dim"]} (same for both)')
    print('=' * 60)
