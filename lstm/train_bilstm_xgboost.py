"""
XGBoost → Bi-LSTM concurrent slowdown prediction.
Stage 1: XGBoost OOF 5-dim resource predictions (pre-cached)
...
# ═══════════ Stage 1: Load XGBoost OOF Predictions ═══════════

def load_resource_cache():
    path = os.path.join(CKPT_DIR, 'oof_xgboost_k5.json')
    with open(path) as f:
        return json.load(f)
...
def main():
    print('=' * 55)
    print('XGBoost → Bi-LSTM (17-dim, Hard Split)')
    print('=' * 55)

    cache = load_resource_cache()
    print(f'[1] XGBoost OOF cache: {len(cache)} queries')
