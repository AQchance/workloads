"""
Single-head Bi-LSTM for pure ICONQ baseline.
No GNN resources → no dual-head → pure ratio prediction.
"""
...
class RatioBiLSTM(nn.Module):
    """Single-head Bi-LSTM for ratio prediction (pure ICONQ baseline)."""
...
def main(trace_file, epochs=200, lr=1e-3, seed=42, prefix=''):
    ...
    model = RatioBiLSTM(input_dim=train_ds.X.shape[2])
